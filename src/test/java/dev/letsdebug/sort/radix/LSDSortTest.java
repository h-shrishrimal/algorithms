...
public class LSDSortTest {
  ...
  void lsdSortTest() throws Exception {
    // copy the data
    String[] input = Arrays.copyOf(SORTED_DATA, SORTED_DATA.length);
    ...
    assertArrayNotEquals(input, SORTED_DATA);
    // method under test
    LSDSort.sort(input);
    Assertions.assertArrayEquals(input, SORTED_DATA);
  }
  ...
  private void assertArrayNotEquals(String[] expected, String[] actual) {
    try {
